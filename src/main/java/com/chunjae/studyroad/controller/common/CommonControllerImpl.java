...
/**
 * 파일, HOME(welcome page) 처리를 담당하는 컨트롤러
 */
public class CommonControllerImpl implements CommonController {
	
	// BaseController 인스턴스
	private static final CommonControllerImpl INSTACE = new CommonControllerImpl();
	
	// 사용 서비스
	private final MemberService memberService = MemberServiceImpl.getInstance();
	
	// 생성자 접근 제한
	private CommonControllerImpl() {
		System.out.println("BaseControllerImpl 생성!");
	}
	
	// 이미 생성한 인스턴스 제공
	public static CommonControllerImpl getInstance() {
		return INSTACE;
	}
